...
describe('memory store', () => {
	...
	describe('sizeCalculation', () => {
		...
		it('calculatedSize and sizeCalcuation must be the same', async () => {
		...
		it('should cache new value and drop the old one(s) if maxSize is reached', async () => {
			const key = 'foo';
			const value = 'bar';
		...
		it('should not cache something greater than maxSize', async () => {
			const key = 'foo';
	...
	describe('when used with wrap() function', () => {
		...
		describe('when cache misses', () => {
			...
			async function getCachedString() {
			...
			async function getCachedNumber() {
