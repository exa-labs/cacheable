...
function clone<T>(object: T): T {
	...
	try {
		...
		// If structuredClone fails, fall back to lodash.clonedeep
		// This handles edge cases like objects with functions or circular references
...
export function memoryStore(arguments_?: MemoryConfig): MemoryStore {
	...
	const isCacheable
    = arguments_?.isCacheable ?? (value => value !== undefined);
